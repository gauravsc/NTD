...
from attention_model_logit_no_encoder import *
import numpy as np
...
def traverse_tree(node, decoder_input, decoder_context, decoder_hidden, encoder_outputs, teacher_forcing, depth, threshold):
	...
	if teacher_forcing:
		...
		p = rd.random()
		compute_p =  1000 + (freq_nodes_min+1)*1.0/float(freq_nodes[node.name]) 
		# print compute_p
		if p < compute_p:  
...
def train(input_variables, target_variables, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, threshold):
	...
	for it in xrange(len(input_variables)):
		...
		decoder_context = Variable(torch.zeros(1, 200))
...
X_train = [X_train[i] for i in xrange(5000)]
Y_train = [Y_train[i] for i in xrange(5000)]
...
X_val = [X_val[i] for i in xrange(1000)]
Y_val = [Y_val[i] for i in xrange(1000)]
