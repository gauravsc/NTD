...
def traverse_tree(node, decoder_input, decoder_context, decoder_hidden, encoder_outputs, teacher_forcing, depth, threshold):
	...
	if teacher_forcing:
		...
		target_new  = decoder_output_new.flatten()
		# print node_to_children[node.name]
		
		for i in node_to_children[node.name]:
			target_new[i] = target[i]
		# target = [target[i] if i in node_to_children[node.name] else decoder_output_new[0,i] for i in xrange(len(target))]		
		
		act_children = [i for i in node_to_children[node.name] if target_new[i] == 1]
		inact_children = [i for i in node_to_children[node.name] if target_new[i] == 0]

		sampled_inact_children = rd.sample(inact_children, min(len(inact_children), 3*len(act_children)))

		# for i in node_to_children[node.name]:
		for i in sampled_inact_children:
			# if target_new[i] == 0:
			# 	if rd.random() < 0.5:
			target_new[i] = decoder_output_new[0,i]

		target = target_new.reshape((1, output_size))
		# target = [[float(t) for t in target_new]]
...
def generate_predictions(X, Y, encoder=None, decoder=None, is_test=0, abstracts=None, threshold=0.5):	
	...
	for i in xrange(len(X)):	
		...
		# if i < 5:
		# 	print "**********************************************"
		# 	print sorted(true_mesh_terms)
		# 	print sorted(pred_mesh_terms)
		# 	print "**********************************************"
...
if load_trained_model:
	...
	for epoch in range(0, n_epochs + 1):
		...
		for i in xrange(len(target_variables)):
			target_variables_sampled.append(rd.sample(target_variables[i], min(14, len(target_variables[i]))))
