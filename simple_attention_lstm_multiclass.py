...
def train(model, X_train, Y_train, X_val, Y_val, nb_epoch=1, batch_size=128):
	...
	for i in xrange(iterations):
		...
		# initial_epoch = int((iterations*batch_size)/X_train.shape[0])
...
def get_mesh_to_seq(list_items):
	mesh_to_seq = {}

	for item in list_items:
		for seq in item['sequence']:	
			mesh_to_seq[seq[0]] = seq[1]
			
	return mesh_to_seq


def get_seq_to_mesh(list_items):
	seq_to_mesh = {}

	for item in list_items:
		for seq in item['sequence']:	
			seq_to_mesh[seq[1]] = seq[0]
...
def get_data(preprocessor):
	...
	cPickle.dump(X_test, open('test_abstract,pkl','w'))
...
def get_attention_weights(model, X_test, Y_test, preprocessor):
	X_test = X_test[0:40]
	Y_test = Y_test[0:40]
	abstracts_test = cPickle.load(open('test_abstract,pkl','r'))[0:40]
	...
	cPickle.dump((attention_output, X_test, Y_test, preprocessor.word_indices_to_words, abstracts_test), open('attention_output.pkl','w'))
...
mesh_to_seq = get_mesh_to_seq(list_items)
cPickle.dump(mesh_to_seq, open('mesh_to_seq.pkl','w'))

seq_to_mesh = get_seq_to_mesh(list_items)
...
model = train(model, X_train, Y_train, X_val, Y_val, nb_epoch=100, batch_size=batch_size)
