...
def build_model(preprocessor):
	...
	x = Embedding(X_vocab_size, X_max_len, input_length=X_max_len, mask_zero=True, weights=preprocessor.init_vectors)(x)
	x = LSTM(hidden_size)(x)
	x = RepeatVector(max_seq_len)(x)

	# decoder network
	for _ in range(num_layers):
	    x = LSTM(hidden_size, return_sequences=True)(x)
	x = TimeDistributed(Dense(y_vocab_size+1))(x)
	y1 = Activation('softmax')(x)
	
	z = Dense(num_labels)(x)
	y2 = Activation('sigmoid')(z)
