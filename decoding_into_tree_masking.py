...
def traverse_tree(node, decoder_input, decoder_context, decoder_hidden, encoder_outputs, teacher_forcing, depth, threshold):
	...
	if teacher_forcing:
		...
		act_children = [i for i in node_to_children[node.name] if target[i] == 1]
		inact_children = [i for i in node_to_children[node.name] if target[i] == 0]

		sampled_inact_children = rd.sample(inact_children, min(len(inact_children), 10*len(act_children)))
		...
		for i in node_to_children[node.name]:
			print decoder_output_before[i], decoder_output_after[i], target[0][i]
	...
	for child in node.children:
		...
		loss += traverse_tree(node.children[child], decoder_input, decoder_context, decoder_hidden, encoder_outputs, teacher_forcing, depth+1, threshold)
...
def train(input_variables, target_variables, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, threshold):
	...
	for it in xrange(len(input_variables)):
		...
		# return
		loss += traverse_tree(root, decoder_input, decoder_context, decoder_hidden, encoder_outputs, teacher_forcing, 0, threshold)
...
def traverse_prediction(decoder_input, decoder_context, decoder_hidden, encoder_outputs, depth, threshold):
	...
	decoder_output, decoder_context, decoder_hidden, decoder_attention = decoder(decoder_input, decoder_context, decoder_hidden, encoder_outputs)
	predictions = (decoder_output > threshold).data[0].cpu().numpy()
...
def generate_predictions(X, Y, encoder=None, decoder=None, is_test=0, abstracts=None, threshold=0.5):	
	...
	if is_test == 1:
		cPickle.dump((mesh_terms_test, mesh_terms_pred), open('predictions_decoder_'+str(len(X_train))+'.pkl','w'))
	return metrics

X_train, Y_train, X_test, Y_test, X_val, Y_val, num_english_words, word2index = get_data()

X_train = [X_train[i] for i in xrange(250)]
Y_train = [Y_train[i] for i in xrange(250)]
...
X_val = [X_train[i] for i in xrange(250)]
Y_val = [Y_train[i] for i in xrange(250)]
...
if load_trained_model:
	...
	for epoch in range(0, n_epochs + 1):
		...
		for i in xrange(len(target_variables)):
			target_variables_sampled.append(rd.sample(target_variables[i], min(4, len(target_variables[i]))))
		...
		# if epoch == 0: continue
		if epoch % print_every == 0 and epoch != 0 and epoch > 0:
